        ...
        if(net_ptr->TI[adr_TI].P){
            ...
            for(int TP1_len = 0; TP1_len < v/2; TP1_len++){
                ...
                do{
                    ...
                    if(net_ptr->TI[adr_TP1].P && net_ptr->TI[adr_TP2].P){
                        if(!suc_sp){
                            ...
                            net_ptr->TI[adr_TI].O = (float*) malloc(TENSOR_NUM * sizeof(float));
                            for(int i = 0; i < TENSOR_NUM; i++) net_ptr->TI[adr_TI].R[i] = net_ptr->TI[adr_TP1].R[i] + net_ptr->TI[adr_TP2].R[i];
                            for(int i = 0; i < TENSOR_NUM; i++) net_ptr->TI[adr_TI].O[i] = net_ptr->TI[adr_TP1].O[i] + net_ptr->TI[adr_TP2].O[i];
            ...
            FPrune(*net_ptr, v, adr_TI); // update outer product
        }
        instance_count++;
