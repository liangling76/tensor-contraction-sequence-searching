...
void MSseq(Network &net){
    ...
    for(int v = 2; v < TENSOR_NUM + 1; v++){
        ...
        do{
            ...
            // select first split case to calculate tensor size (S) and row vector (R) of TI
            adr_TP1 = adr_TI_a[0];
            adr_TP2 = adr_TI - adr_TP1;

            // calculate SO in first split case
            SO = net.TI[adr_TP2].R[idx_TI_a[0]];
            for(int i = 1; i < v; i++) SO += net.TI[adr_TP1].R[idx_TI_a[i]];
...
void run(Network &net) {
    ...
    cout << "execution time:\t" << chrono::duration_cast<std::chrono::microseconds>(stop - start).count() / 1e6 << " s\n\n";
